...
const AppointmentController = {
  ...
  async getAllAppointmentDataByNumber(req, res) {
    ...
    try {
      const appointment =
        await AppointmentService.getAllAppointmentDataByNumber(number);
      if (!appointment)
        return res.status(404).json({ error: "Appointment not found" });
      res.status(200).json(appointment); // Devuelve la cita encontrada
    } catch (error) {
  ...
  async getAllAppointmentsByEmail(req, res) {
    ...
    try {
      ...
      const appointments = await AppointmentService.getAllAppointmentsByEmail(
        email
      );
      res.status(200).json(appointments); // Devuelve la lista de citas del cliente
    } catch (error) {
  ...
  async getAllAppointmentsByBusinessId(req, res) {
    ...
    try {
      const appointments =
        await AppointmentService.getAllAppointmentsByBusinessId(businessId);
  ...
  async updateStatus(req, res) {
    const { id } = req.params; // `id` es el ID de la cita que se quiere actualizar
    const { status } = req.body; // `status` es el nuevo estado que se desea establecer

    console.log("he recibido la llamada: ", status, id);
    if (!status) {
    ...
    try {
      ...
      res.status(200).json(updatedAppointment); // Devuelve la cita actualizada con el nuevo estado
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: error.message }); // Manejo de errores
    }
